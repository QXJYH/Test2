...
public class InventoryControllerV1 : ControllerBase
{
		...
		string? sortOrder = "desc", string? assetType = null)
	{
		...
		var items = await services.inventory.GetCollectibleInventory(userId, actualAssetType, sortOrder, limit, offset);
		var resultList = items.ToList();
		var totalRap = resultList.Sum(item => item.recentAveragePrice);
		
		return new RobloxCollectionPaginated<CollectibleItemEntry>()
		{
			nextPageCursor = resultList.Count >= limit ? (offset + limit).ToString() : null,
