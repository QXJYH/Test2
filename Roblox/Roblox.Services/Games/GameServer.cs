...
public class GameServerService : ServiceBase
{
	...
	public void ShutDownServer(string serverId)
	{
		...
		{
			Console.WriteLine("[ERROR] serverId is null or empty!");
		...
		try
		{
			...
			Console.WriteLine($"[DEBUG] Attempting to shut down server with serverId: {serverId}");

			string placeJobId = serverId; // Ideally, validate this value to ensure it's not null or unexpected

			// Get the associated PlaceId by JobId
			long placeId = GetPlaceIdByJobId(serverId);
			if (placeId == 0)
			{
				Console.WriteLine($"[ERROR] Unable to find placeId for serverId: {serverId}");
				return; // Or throw an exception if preferred
			}

			// Check if the process exists in the jobRccs dictionary
			if (!jobRccs.TryGetValue(placeJobId, out Process rccProcess))
			{
				Console.WriteLine($"[ERROR] Process for serverId {serverId} not found in jobRccs dictionary.");
				return;
			}

			// Gracefully stop the server (soft kill logic can be added here later)
			if (!rccProcess.HasExited)
			{
				Console.WriteLine($"[INFO] Soft killing server process for serverId: {serverId}");
				rccProcess.Kill(); // Replace with soft kill when ready
			}
			else
			{
				Console.WriteLine($"[INFO] Server process for serverId {serverId} has already exited.");
			}

			// Clean up associated dictionaries
			...
			// Remove players from the place (optional - ensure you handle this correctly)
		...
		catch (Exception ex)
		{
			...
			Console.WriteLine($"[ERROR] An error occurred while shutting down the server: {ex.Message}");
		}
	}
	...
	public async Task<DateTime> GetLastServerPing(string serverId)
	{
		...
		// Check if result is null (no data found for the given serverId)
		if (result == null || result.updated_at == null)
		{
			// Log and throw or return a default DateTime, depending on your use case
			Console.WriteLine($"[ERROR] No data found for serverId: {serverId}");
			throw new InvalidOperationException($"No data found for serverId: {serverId}");
			
			// Alternatively, return a default DateTime if you don't want to throw an error:
			// return default(DateTime);
		}

		// Return the updated_at field if it exists
    ...
    */
    ...
    public async Task<GameServerGetOrCreateResponse> GetServerForPlace(long placeId)
    {
		...
		private static readonly int[] AllowedNetworkPorts = { 100, 101 };
    ...
    public async Task DeleteOldGameServers()
    {
        ...
        */
		...
		public async Task CreateManualServer(string serverId, long placeId, int port)
	{
		await db.ExecuteAsync(
			@"INSERT INTO asset_server 
			  (id, asset_id, ip, port, server_connection, created_at, updated_at) 
			  VALUES 
			  (:id::uuid, :asset_id, '127.0.0.1', :port, 'manual', now(), now())",
			new
			{
				id = serverId,
				asset_id = placeId,
