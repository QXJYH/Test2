...
public class EconomyControllerV2 : ControllerBase
{
	...
	public async Task<IActionResult> CreateCurrencyExchangeOrder([Required, FromBody] CreateExchangeOrderRequest request)
	{
		FeatureCheckCurrencyExchange();
		...
		try
		{
			...
			if (request.sourceCurrency == CurrencyType.Robux)
			{
				...
				newTickets += (long)(request.amount * exchangeRate);
			}
			else
			{
				newTickets -= request.amount;
				newRobux += (long)(request.amount / exchangeRate);
			}
			...
			return Ok(new 
			{
				newRobuxBalance = newRobux,
				newTicketsBalance = newTickets
			});
		}
		catch (Exception ex)
		{
			return StatusCode(500, new { message = "Currency exchange failed", error = ex.Message });
		}
	}
