...
const NavSideBar = props => {
      ...
      try {
        ...
        if (response.data.isStaff) {
          const [pendingIcons, pendingAssets, pendingGroupIcons] = await Promise.all([
            request('GET', getFullUrl('admin-api', '/api/icons/pending-assets')),
            request('GET', getFullUrl('admin-api', '/api/assets/pending-assets')),
            request('GET', getFullUrl('admin-api', '/api/groups/pending-icons'))
          ]);
  ...
  return <div className={s.container}>
    <div className={s.card} style={{ paddingTop: paddingTop }}>
      ...
      <LinkEntry name='Downloads' url='https://bt.zawg.ca/downloads?sitetest=true' icon='icon-nav-blog' />
