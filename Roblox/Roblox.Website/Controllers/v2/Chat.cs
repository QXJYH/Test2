...
public class ChatControllerV2 : ControllerBase
{
	...
	public async Task<dynamic> GetAuthenticatedUserConversations(int pageNumber, int pageSize)
	{
		try
		{
			...
			// Ensure user is authenticated (safeUserSession should not be null)
			var userId = safeUserSession?.userId;
			if (userId == null)
			{
				throw new RobloxException(401, 0, "User cannot fetch messages. Unauthorized.");
			}

			// Get conversations for the user
			var conversations = await services.chat.GetUserConversations(userId.Value);
			...
			foreach (var item in conversations.Skip(offset))
			{
				...
				// Fetch participants, names, unread message status
				...
				var hasUnRead = await services.chat.DoesHaveUnreadMessages(item.id, userId.Value);
		...
		catch (RobloxException ex) when (ex.Message.Contains("Unauthorized"))
		{
			// Return a user-friendly message if the user is unauthorized
			return new { error = "User is not authorized to view conversations." };
		}
		catch (Exception ex)
		{
			// Catch any other general errors and log them
			// Optionally log the error here
			return new { error = "An error occurred while fetching conversations." };
		}
	}
